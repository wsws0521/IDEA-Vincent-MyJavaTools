...
BEGIN
	...
	# 删除被插表索引
	IF NOT EXISTS(SELECT * FROM information_schema.statistics WHERE table_name='vd_a_pay_flow_2015' AND index_name='IDX_PAY_FLOW_METER_ID') THEN
		ALTER table vd_a_pay_flow_2015 DROP INDEX IDX_PAY_FLOW_METER_ID;
	END IF;
	IF NOT EXISTS(SELECT * FROM information_schema.statistics WHERE table_name='vd_a_pay_flow_2015' AND index_name='IDX_PAY_FLOW_DS_ID') THEN
		ALTER table vd_a_pay_flow_2015 DROP INDEX IDX_PAY_FLOW_DS_ID;
	END IF;
	IF NOT EXISTS(SELECT * FROM information_schema.statistics WHERE table_name='vd_a_pay_flow_2015' AND index_name='IDX_PAY_FLOW_ORG_ID') THEN
		ALTER table vd_a_pay_flow_2015 DROP INDEX IDX_PAY_FLOW_ORG_ID;
	END IF;
	IF NOT EXISTS(SELECT * FROM information_schema.statistics WHERE table_name='vd_a_pay_flow_2015' AND index_name='IDX_PAY_FLOW_OBJ_ID') THEN
    ...
    DELETE FROM vd_a_pay_flow_2015;

    # 1-插入 收费明细(正常)
    SET start_line = 0;
    while start_line < 12293116 do
        SET @strsql = CONCAT('INSERT INTO vd_a_pay_flow_2015
            ...
            LEFT JOIN uap_organization consorg ON cons.org_no = consorg.no ', 'LIMIT ', start_line, ',', offset);
    ...
    # 重建索引
    ALTER table vd_a_pay_flow_2015 ADD INDEX IDX_PAY_FLOW_METER_ID(METER_ID);
    ALTER table vd_a_pay_flow_2015 ADD INDEX IDX_PAY_FLOW_DS_ID(DS_ID);
    ALTER table vd_a_pay_flow_2015 ADD INDEX IDX_PAY_FLOW_ORG_ID(ORG_ID);
    ALTER table vd_a_pay_flow_2015 ADD INDEX IDX_PAY_FLOW_OBJ_ID(OBJ_ID);
