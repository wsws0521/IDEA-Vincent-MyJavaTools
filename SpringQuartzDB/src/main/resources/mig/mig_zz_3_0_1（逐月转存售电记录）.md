...
where table_schema = 'centlec' and table_name='表名';
```
-- 后来业务表不分年份表，七年集中于每一张业务表，所以需要【新增分区】
-- 导完七年再去新增分区，新增单独一年的分区需要 16.5min，一次性新增3年的分区反而更快4min，可能因为每一行数据都可以安家到一个分区，不需要排除吧
```sql
ALTER TABLE ********** REORGANIZE PARTITION PMAX INTO ( -- alter table ********* partition by range(TO_DAYS(TV)) -- 重建分区
    -- 2016
	PARTITION P201601 VALUES LESS THAN (TO_DAYS('2016-02-01')) ENGINE = InnoDB,
    PARTITION P201602 VALUES LESS THAN (TO_DAYS('2016-03-01')) ENGINE = InnoDB,
    PARTITION P201603 VALUES LESS THAN (TO_DAYS('2016-04-01')) ENGINE = InnoDB,
