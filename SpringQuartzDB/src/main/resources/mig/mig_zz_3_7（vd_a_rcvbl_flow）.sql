...
BEGIN
	...
	WHERE flow.charge_remark = 'migrate normal' AND sdjl.ISFREE= 0 AND sdjl.OCD_MONEY > 0 -- 1-【正常收费】{电价电费应收}
	...
	LEFT JOIN tmp_sdjl_2015 sdjl ON flow.orderid = sdjl.ORDERSID
	LEFT JOIN vd_e_calc_pp_parm_2015 pp ON flow.orderid = pp.ORDERID
	WHERE flow.charge_remark = 'migrate normal' AND sdjl.ISFREE= 0 AND sdjl.FP_VAL3 > 0 -- 2-【正常收费】{计费项应收}
	...
	LEFT JOIN tmp_sdjl_2015 sdjl ON flow.orderid = sdjl.ORDERSID
	LEFT JOIN vd_e_calc_pp_parm_2015 pp ON flow.orderid = pp.ORDERID
	WHERE flow.charge_remark = 'migrate normal' AND sdjl.ISFREE= 0 AND sdjl.OCD_DEBT > 0 -- 3-【正常收费】{债务应收}
	...
	LEFT JOIN tmp_sdjl_2015 sdjl ON flow.orderid = sdjl.ORDERSID
	LEFT JOIN vd_e_calc_pp_parm_2015 pp ON flow.orderid = pp.ORDERID
	WHERE flow.charge_remark = 'migrate normal' AND sdjl.ISFREE= 1 AND sdjl.OCD_MONEY > 0 -- 4-【FBE收费】{+OCD_MONEY}
	...
	LEFT JOIN tmp_sdjl_2015 sdjl ON flow.orderid = sdjl.ORDERSID
	LEFT JOIN vd_e_calc_pp_parm_2015 pp ON flow.orderid = pp.ORDERID
	WHERE flow.charge_remark = 'migrate normal' AND sdjl.ISFREE= 1 AND sdjl.OCD_MONEY > 0; -- 5-【FBE收费】{-OCD_MONEY}
