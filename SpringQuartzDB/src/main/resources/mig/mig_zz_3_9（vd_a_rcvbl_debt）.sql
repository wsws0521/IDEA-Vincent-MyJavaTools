...
-- 	DECLARE total int default 60000000; -- 必须是offset的倍数（待定）
    ...
    # vd_a_rcvbl_debt_2015.RCVBL_AMT_ID
    IF NOT EXISTS(SELECT * FROM information_schema.statistics WHERE table_name='vd_a_rcvbl_debt_2015' AND index_name='index_vd_a_rcvbl_debt_rcvblamtid') THEN
		ALTER table vd_a_rcvbl_debt_2015 ADD INDEX index_vd_a_rcvbl_debt_rcvblamtid(RCVBL_AMT_ID); -- 1s 插实收小弟的时候需要用到
	END IF;

--     while start_line < total do
-- 	    -- 1-【正常收费】{电价电费应收}
--         SET @strsql = CONCAT('INSERT INTO vd_a_rcvbl_debt_2015
--                                     (`LESSEE_ID`, `RCVBL_DEBT_ID`, `RCVBL_AMT_ID`, `DEBT_ID`, `DEBT_TYPE`, `AMOUNT`, `REMAIN_DEBT`, `DEBT_VALUE`,
--                                     `EXPIRED_DATE`, `RCVED_AMT`, `ORG_ID`, `RCVBL_AMT`, `TV`)
--                                 SELECT
--                                     2, AMI_GET_SEQUENCE(''SEQ_VD_A_RCVBL_DEBT''), -- RCVBL_DEBT_ID
...
--                                     hzjl.paymoney, -- RCVBL_AMT
--                                     hzjl.paydate -- 手动插入TV字段，应用于分区（该值不应为空）
--                                 FROM vd_a_rcvbl_flow_2015 rcvblflow INNER JOIN (select RCVBL_AMT_ID from vd_a_rcvbl_flow_2015 where amt_type = ''11'' limit ', start_line, ',', offset, ') tmprcvblflow ON rcvblflow.RCVBL_AMT_ID = tmprcvblflow.RCVBL_AMT_ID
--                                 INNER JOIN tmp_hzjl hzjl ON rcvblflow.orderid = hzjl.orderid -- 一个售电订单可能偿还2种债务
--                                 LEFT JOIN vd_a_user_debt debt ON hzjl.DEBTID = debt.debt_from_obj_id;');
