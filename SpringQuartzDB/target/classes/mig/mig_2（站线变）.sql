	...
	IF NOT done THEN
		...
		CASE
			WHEN var_typeFlag = 2 THEN
				# 变电站
				SET var_zxbOrgNo = (SELECT NO FROM UAP_ORGANIZATION WHERE CODE = CONCAT('dw_',mainKey));
				...
				SELECT
					(select AMI_GET_SEQUENCE('S_AMI_FILE') from dual), object_name, CONCAT('zxb_',object_id), NULL, NULL, NULL, NULL,
					var_zxbOrgNo, '01', SYSDATE(), NULL, NULL, NULL, NULL, NULL, NULL
				FROM tmp_zxb WHERE object_id = mainKey;
				# 将新ID更新至临时表
				UPDATE tmp_zxb SET NEWID = (select subs_id from a_grid_subs where subs_no = CONCAT('zxb_',mainKey)) where object_id = mainKey;
			WHEN var_typeFlag = 3 THEN
				# 线路
				SET var_zxbFatherId = (SELECT FatherId FROM tmp_zxb WHERE object_id = mainKey);
				SET var_zxbOrgNo = (SELECT NO FROM UAP_ORGANIZATION WHERE CODE = CONCAT('dw_',var_zxbFatherId));
				...
				SELECT
					...
					CONCAT('zxb_',object_id), object_name, NULL, NULL, 'AC00101', '02', SYSDATE(),
				...
				SELECT
					(select line_id from a_grid_line where line_no = CONCAT('zxb_',mainKey)),
					(select subs_id from a_grid_subs where subs_no = CONCAT('zxb_',var_zxbFatherId)), SYSDATE(), 0
				FROM DUAL;
				# 将新ID更新至临时表
				UPDATE tmp_zxb SET NEWID = (select line_id from a_grid_line where line_no = CONCAT('zxb_',mainKey)) where object_id = mainKey;
			WHEN var_typeFlag = 4 THEN
				# 变压器
				SET var_zxbFatherId = (SELECT FatherId FROM tmp_zxb WHERE object_id = mainKey);
				SET var_zxbOrgNo = (SELECT NO FROM UAP_ORGANIZATION WHERE CODE = CONCAT('dw_',(SELECT fatherId FROM tmp_zxb WHERE object_id = var_zxbFatherId)));
				...
				SELECT
					(select AMI_GET_SEQUENCE('S_AMI_FILE') from dual), var_zxbOrgNo,
					NULL, CONCAT('zxb_',object_id), object_name, NULL, SYSDATE(), NULL, NULL, NULL, NULL, NULL, NULL, NULL, '02', '01',
				...
				SELECT
					(select line_id from a_grid_line where line_no = CONCAT('zxb_',var_zxbFatherId)),
					(select tf_id from a_grid_transformer where tf_no = CONCAT('zxb_',mainKey)), SYSDATE(), 0
				FROM DUAL;
				# 将新ID更新至临时表
				UPDATE tmp_zxb SET NEWID = (select tf_id from a_grid_transformer where tf_no = CONCAT('zxb_',mainKey)) where object_id = mainKey;
		END CASE;
	END IF;
